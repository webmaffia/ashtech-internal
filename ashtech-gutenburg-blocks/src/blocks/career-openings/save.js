import { RichText } from '@wordpress/block-editor';

export default function save({ attributes }) {
    const { label, title, howToApplyTitle, howToApplyText } = attributes;

    return (
        <section className="career-openings">
            <div className="career-openings__background"></div>
            <div className="career-openings__container">
                <div className="career-openings__header">
                    <RichText.Content tagName="p" className="career-openings__label animate-text" value={label} />
                    <RichText.Content tagName="h2" className="career-openings__title animate-text" value={title} />
                </div>
                
                <div className="career-openings__list">
                    <div className="job-card">
                        <div className="job-card__info">
                            <h3 className="job-card__title">Site Engineer</h3>
                            <div className="job-card__tags">
                                <span className="job-card__tag"><img src="assets/images/career/icon-category.svg" alt="" />Construction</span>
                                <span className="job-card__tag"><img src="assets/images/career/icon-location.svg" alt="" />Pune</span>
                                <span className="job-card__tag"><img src="assets/images/career/icon-clock.svg" alt="" />10 Oct 2025</span>
                            </div>
                        </div>
                        <button className="btn btn--secondary career-openings__apply">
                            <span>Apply Now</span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="8" height="17" viewBox="0 0 8 17" fill="none">
                                <path d="M0.499976 0.499169C0.499976 0.499169 5.36133 5.36052 6.3336 6.33279C7.30588 7.30507 7.30588 9.24962 6.3336 10.2219C5.36134 11.1942 0.499977 16.0555 0.499977 16.0555" stroke="#0C0D0D" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                        </button>
                    </div>
                    <div className="job-card">
                        <div className="job-card__info">
                            <h3 className="job-card__title">Project Manager</h3>
                            <div className="job-card__tags">
                                <span className="job-card__tag"><img src="assets/images/career/icon-category.svg" alt="" />Infrastructure</span>
                                <span className="job-card__tag"><img src="assets/images/career/icon-location.svg" alt="" />Mumbai</span>
                                <span className="job-card__tag"><img src="assets/images/career/icon-clock.svg" alt="" />08 Oct 2025</span>
                            </div>
                        </div>
                        <button className="btn btn--secondary career-openings__apply">
                            <span>Apply Now</span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="8" height="17" viewBox="0 0 8 17" fill="none">
                                <path d="M0.499976 0.499169C0.499976 0.499169 5.36133 5.36052 6.3336 6.33279C7.30588 7.30507 7.30588 9.24962 6.3336 10.2219C5.36134 11.1942 0.499977 16.0555 0.499977 16.0555" stroke="#0C0D0D" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                        </button>
                    </div>
                    <div className="job-card">
                        <div className="job-card__info">
                            <h3 className="job-card__title">Marketing Executive</h3>
                            <div className="job-card__tags">
                                <span className="job-card__tag"><img src="assets/images/career/icon-category.svg" alt="" />Sales & Marketing</span>
                                <span className="job-card__tag"><img src="assets/images/career/icon-location.svg" alt="" />Pune</span>
                                <span className="job-card__tag"><img src="assets/images/career/icon-clock.svg" alt="" />05 Oct 2025</span>
                            </div>
                        </div>
                        <button className="btn btn--secondary career-openings__apply">
                            <span>Apply Now</span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="8" height="17" viewBox="0 0 8 17" fill="none">
                                <path d="M0.499976 0.499169C0.499976 0.499169 5.36133 5.36052 6.3336 6.33279C7.30588 7.30507 7.30588 9.24962 6.3336 10.2219C5.36134 11.1942 0.499977 16.0555 0.499977 16.0555" stroke="#0C0D0D" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                        </button>
                    </div>
                </div>
                
                <div className="career-openings__how-to-apply">
                    <RichText.Content tagName="h3" className="career-openings__apply-title animate-text" value={howToApplyTitle} />
                    <RichText.Content tagName="p" className="career-openings__apply-text animate-text" value={howToApplyText} />
                </div>
            </div>
        </section>
    );
}
